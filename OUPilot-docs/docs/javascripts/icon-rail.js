/* ───────────────────────────────────────────────────────────────────────────
   Icon-Rail + Flyout-Panel — für OUPilot adaptiert (2026-07-06).

   Basis: neutrale Vorlage, portiert aus den anderen Lucent-Doku-Sites.

   ADAPT-Marker gesetzt:
   - APP_VERSION, HEADER_PREFIX, LS_KEY, isHomePage-Regex
   - INFO_HTML Stack (WPF / PowerShell 5.1 / ActiveDirectory / MECM)
   - Status-Badge (nur Version — OUPilot ohne pytest-Suite)
   - Architektur-/Roadmap-Badge → images/mermaid/*.svg
   ─────────────────────────────────────────────────────────────────────────── */
(function () {
  'use strict';

  // ── Versionen ──────────────────────────────────────────────────────────
  const APP_VERSION   = '1.4.0';                        // ADAPT: bei Release anpassen
  const TEST_COUNT    = '';                             // ADAPT: OUPilot hat keine pytest-Suite -> Badge zeigt nur Version
  const TEST_DATE     = '2026-07-06';                   // ADAPT: bei Release anpassen
  const HEADER_PREFIX = `OUPilot v${APP_VERSION}`;
  ...
  const LS_KEY = 'oupilot-rail.flyoutIdx';  // ADAPT: eindeutiger Key je Projekt
  const INFO_IDX = -2;
  let _currentIdx = -1;
  let _sections = [];
    ...
    '<div class="adb-rail-flyout__info">' +
      ...
      '<section class="adb-info-block">' +
        ...
        '<table class="adb-info-stack">' +
          '<tr><td>OUPilot</td><td>v' + APP_VERSION + '</td></tr>' +
          '<tr><td>Windows PowerShell</td><td>5.1 (Desktop)</td></tr>' +
          '<tr><td>UI</td><td>WPF / XAML</td></tr>' +
          '<tr><td>Verzeichnisdienst</td><td>ActiveDirectory / ADSI</td></tr>' +
          '<tr><td>Deployment</td><td>MECM (Collections)</td></tr>' +
          '<tr><td>Store</td><td>JSON (objectGUID)</td></tr>' +
  ...
  function updateHeaderTitle() {
    ...
    // Startseite (kein H1 bzw. OUPilot-H1) -> nur Prefix ohne Doppelnennung
    const isHomePage = /oupilot/i.test(pageTitle) || !pageTitle;  // ADAPT
  ...
  function archSvgUrl() {
    const s = document.querySelector('script[src*="javascripts/icon-rail.js"]');
    const base = s ? s.src.replace(/javascripts\/icon-rail\.js.*$/, '') : '/';
    return base + 'images/mermaid/referenz-architektur-1.svg';
  }
  function closeArch() {
  ...
  function openArch() {
    ...
    if (!ov) {
      ...
      ov.innerHTML =
        '<button class="adb-lightbox-close" aria-label="Schliessen">&times;</button>' +
        '<div class="adb-lightbox-content"><img class="adb-lightbox-img" src="' +
        archSvgUrl() + '" alt="OUPilot-Architektur: UI · Core-Module · Active Directory"></div>' +
        '<div class="adb-lightbox-caption">Architektur — UI · Core-Module · Active Directory</div>';
      document.body.appendChild(ov);
      ov.addEventListener('click', function (e) {
        if (e.target === ov || e.target.classList.contains('adb-lightbox-close')) {
          closeArch();
  ...
  function addStatusBadge() {
    ...
    badge.title = `OUPilot v${APP_VERSION} — WPF-App (Windows PowerShell 5.1) zum Hinzufügen von Rechnern zu ` +
      'AD-Software-Gruppen (MECM-Deployment). AD-Lesen/-Schreiben über Modul/ADSI mit Mock-Fallback, stabile ' +
      'objectGUID-Identität, WhatIf + Bestätigung, konfigurierbare Feld-Map.';
    const suffix = TEST_COUNT ? ' · ' + TEST_COUNT + ' Tests' : '';
    badge.innerHTML = '<span class="adb-status-badge__dot" aria-hidden="true"></span>v' + APP_VERSION + suffix;
